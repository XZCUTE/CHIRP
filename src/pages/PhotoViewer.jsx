  ...
  return (
    <div className={`photo-viewer-layout ${isModal ? 'modal-mode' : ''}`}>
      <div className="photo-viewer-media">
        ...
        {hasImages && (
          <div className="modal-image-wrapper" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%' }}>
            <img
      ...
      <aside className="photo-viewer-sidebar">
        ...
        <div className="post-actions photo-sidebar-actions" style={{ justifyContent: 'space-around', padding: '10px 0', borderTop: '1px solid var(--capy-border)', borderBottom: '1px solid var(--capy-border)' }}>
          <button 
            className={`action-btn like-btn ${post.userVotes && user && post.userVotes[user.uid] === 1 ? 'active' : ''}`}
            onClick={() => handleVote(1)}
            title="Like"
            style={{ background: 'transparent', borderRadius: 0, width: 'auto', height: 'auto', padding: '8px' }}
          >
            ...
            <span style={{ marginLeft: '4px' }}>{post.score || 0}</span>
          </button>
          ...
          <button 
            ...
            onClick={() => document.getElementById('photo-comment-input')?.focus()}
            title="Comment"
            style={{ background: 'transparent', borderRadius: 0, width: 'auto', height: 'auto', padding: '8px' }}
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {post.comments > 0 && <span style={{ marginLeft: '4px' }}>{post.comments}</span>}
          </button>
          ...
          >
            ...
            {post.shares > 0 && <span style={{ marginLeft: '4px' }}>{post.shares}</span>}
          </button>
